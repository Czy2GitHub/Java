...
public class PurchaseGoods {
	...
	public  void outPutAll() {
		...
		for(int j = 0; j<3;j++) {
			...
			System.out.println("图书名字"+"\t\t"+book[i].getName()+"\t\t"+"商品数量"+score[i]+"\t\t"+"商品总价"+book[i].getPrice()*score[i]+"\t\t"+"剩余库存"+(book[i].getStorage()-score[i]));
			ord.setTotal(ord.getTotal() + book[i].getPrice()*score[i]);
		}
	}
}
